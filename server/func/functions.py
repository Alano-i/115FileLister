...
def sortout_filename(filename, series, fill_num):
    ...
    try:
        ...
        logger.error(f"处理 ['{filename_text_ori}'] 文件名时出错了: {e}")
...
def write_json_file(file_path, json_data):
    for i in range(3):
        try:
            ...
            logger.error(f'{i+1}/3 次写入新json数据到文件出错，原因: {e}')
            time.sleep(3)
            continue
...
def write_xml_file(file_path, xml_data):
    ...
    for i in range(3):
        try:
            ...
            logger.error(f'{i+1}/3 次写入新xml数据到文件出错，原因: {e}')
            time.sleep(3)
            result = False
    return result
...
def read_json_file(file_path):
    ...
    for i in range(3):
        try:
            ...
            logger.error(f'{i+1}/3 次读取json文件出错，原因: {e}')
            time.sleep(3)
            continue
    return json_data
...
def read_txt_file(file_path):
    ...
    for i in range(3):
        try:
            ...
            logger.error(f"{i+1}/3 次读取 ['{file_path}'] 文件出错，原因: {e}")
            time.sleep(3)
            continue
    return txt_data
...
def hard_link(src, dst):
    ...
    try:
        ...
        if os.path.isfile(src):
            logger.error(f"硬链接 ['{src}'] -> ['{dst}'] 出错，跳过, 原因：{e}")
        else:
            logger.error(f"硬链接 ['{src}'] -> ['{dst}'] 出错，跳过")
        work_flag = False
    return work_flag
...
def light_link(src, dst):
    ...
    try:
        ...
        logger.error(f"软链接 ['{src}'] -> ['{dst}'] 出错，原因: {e}")
        result = False
    return result
...
def hlink(src_base_path, dst_base_path):
    ...
    logger.info(f"已完成 ['{src_base_path}'] -> ['{dst_base_path}'] 链接")
    return result
...
def get_audio_files(directory):
    ...
    sorted_audio_files = sorted(
        audio_files, key=lambda x: pinyin_sort_key(os.path.basename(x)))
...
def read_abs_file(audio_path):
    ...
    if metadata:
        ...
        try:
            ...
            pass
        try:
            publishedYear = int(metadata.split("publishedYear=")[
                                1].split("\n")[0].strip())
        except Exception as e:
            pass
        try:
            ...
            if genres:
                genres = genres.replace("Audiobook, ", "").replace(
                    "Audiobook,", "").replace("Audiobook", "").strip(",").strip()
        except Exception as e:
            pass
        try:
            ...
            description = metadata[description_start:description_end].replace(
                "[DESCRIPTION]", "").strip()
    ...
    if not title or not authors or '.' in title or '-' in title:
        title_dir, authors_dir, reader_dir = get_bookname_and_author(
...
def get_audio_info_all(audio_path, book_title, podcast_author, reader, pub_year, podcast_category, podcast_summary):
    ...
    title_abs, authors_abs, reader_abs, publishedYear_abs, genres_abs, description_abs = read_abs_file(
        audio_path)
...
def move_dir_with_copy_del(src_dir, dst_dir):
    ...
    try:
        ...
        logger.error(f"将 [{src_dir}] -> [{dst_dir}] 先复制再删除失败，原因：{e}")
...
def create_podcast_flag_file(audio_path):
    try:
        if os.path.exists(audio_path):
            ...
            logger.error(f" 目录 {audio_path} 不存在，跳过生成已处理标识文件")
    except Exception as e:
        logger.error(f" 生成已处理标识文件失败，原因：{e}")
...
def update_sub_json(file_path, album_id, remove=False, cover_url='', book_title='', author='', reader='', ep_start=1, audio_start=0, audio_end=0, index_on=False, index_offset=0):
    if not album_id:
        return False
    try:
        # 读取原始JSON文件内容
        if os.path.exists(file_path):
            ...
            original_data = {}
        # 如果需要删除项
        if remove and album_id in original_data:
            original_data.pop(album_id)
            ...
            logger.info(f'接收到订阅信息{new_content_details}')
            # 更新或添加内容
            original_data[album_id] = new_content_details
            # 将新内容放在最前面
            # item = original_data.pop(album_id)
            # updated_data = {album_id: item, **original_data}

            # 将新内容始终放在最前面
            updated_data = {album_id: new_content_details, **original_data}
        # 将更新后的内容写回到同一文件中
        with open(file_path, 'w', encoding='utf-8') as file:
            json.dump(updated_data, file, indent=4, ensure_ascii=False)
        logger.info(f'{file_path} 订阅文件已更新')
        return True
    except Exception as e:
        logger.error(f"保存数据到json文件出错，原因：{e}")
        return False
